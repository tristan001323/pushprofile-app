    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {/* Section Favoris - Preview */}
          <div className="mt-8">
            <div className="flex items-center justify-between px-4 py-2">
              <span className="text-xs font-bold uppercase tracking-widest text-gray-500">Favoris</span>
              {(favoriteSearches.length > 0 || favoriteJobs.length > 0) && (
                <Link href="/favorites" onClick={handleLinkClick} className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">
                  Voir tout
                </Link>
              )}
            </div>
            ...
            <div className="mt-2 space-y-1">
              {/* Apercu des favoris (max 3) */}
              {favoriteSearches.slice(0, 2).map((search) => (
                <Link key={search.id} href={`/searches/${search.id}`} onClick={handleLinkClick}>
                  <div className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-white/5 cursor-pointer group transition-colors">
              ...
              {favoriteJobs.slice(0, 2).map((job) => (
              ...
              {/* Lien vers la page complete si plus de favoris */}
              {(favoriteSearches.length > 2 || favoriteJobs.length > 2) && (
                <Link href="/favorites" onClick={handleLinkClick}>
                  <div className="px-4 py-2 text-center">
                    <span className="text-xs text-gray-500 hover:text-indigo-400 transition-colors">
                      + {(favoriteSearches.length - 2) + (favoriteJobs.length - 2)} autres favoris
                    </span>
                  </div>
                </Link>
              )}
